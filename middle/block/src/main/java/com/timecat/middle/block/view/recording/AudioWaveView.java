...
public class AudioWaveView extends View {
    ...
    public void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            mOffset = ta.getInt(R.styleable.waveView_view_waveOffset, dip2px(context, -11));
            mWaveColor = ta.getColor(R.styleable.waveView_view_waveColor, Color.WHITE);
            mWaveCount = ta.getInt(R.styleable.waveView_view_waveCount, 2);
            ta.recycle();
        }
            ...
            while (mIsDraw) {
                ...
                synchronized (mVolumeList) {
                    if (mVolumeList.size() != 0) {
                        try {
                            myVolumeList = deepCopy(mVolumeList);// 保存  接收数据
                        } catch (Exception e) {
                            e.printStackTrace();
                            continue;
                        }
