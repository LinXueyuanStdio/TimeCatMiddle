...
/**
 * Created by 张启 on 2017/4/7. Helper class to set line spacing and solve cursor issues.
 */
public class LineSpacingHelper {

    /**
     * Even if we changed line spacing of an EditText, it won't work when we press enter and start a
     * new line. At this situation, the line above last(newly added) will keep original line height,
     * which is a bug above Lollipop. {@see http://stackoverflow.com/questions/36075205/android-textview-edittext-new-line-spacing}
     * <p>
    ...
    public static void setTextCursorDrawable(
        ...
        if (!DeviceUtil.hasP()){
            setTextCursorDrawableBeforeP(et, cursorColor, cursorWidth, normalLineCursorHeightVary, lastLineCursorHeightVary);
        } else {
            setTextCursorDrawableAfterP(et, cursorColor, cursorWidth, normalLineCursorHeightVary, lastLineCursorHeightVary);
        }
    }

    public static void setTextCursorDrawableBeforeP(
            EditText et, int cursorColor, int cursorWidth,
            int normalLineCursorHeightVary, int lastLineCursorHeightVary) {
        try {
    ...
    public static void setTextCursorDrawableAfterP(
            EditText et, int cursorColor, int cursorWidth,
            int normalLineCursorHeightVary, int lastLineCursorHeightVary) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
//            Drawable d = et.getTextCursorDrawable();
            Drawable d = new LineSpacingCursorDrawable(
                    et, cursorColor, cursorWidth,
                    normalLineCursorHeightVary, lastLineCursorHeightVary);
            et.setTextCursorDrawable(d);
        }
    }
    ...
    private static class LineSpacingCursorDrawable extends ShapeDrawable {
        ...
        public void setBounds(int left, int top, int right, int bottom) {
            ...
            int heightVary = cursorLine != lineCount - 1 ? mNormalLineHeightVary : mLastLineHeightVary;
